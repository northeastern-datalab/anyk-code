...
parser.add_argument('-e', nargs='+', dest="e_list", default=[], help="list of (milli-)epsilon values (x-axis)")
...
e_list = [int(e) for e in arg_results.e_list]
...
for i in range(len(algorithms)):
	...
	for e in e_list:
		...
		if (alg.startswith("QEq_")):
			# QEq is the same as Batch for a binary join
		...
		if (alg == "psql"):
			# Read file
			try:
				fp1 = open(inFileName + "_e" + str(e) + "_k" + str(k_to_plot) + "_" + alg + ".out")
			except IOError:
				times[alg].append(None)
				times_std[alg].append(None)
				continue
			...
			# Read file
			try:
				fp1 = open(inFileName + "_e" + str(e) + "_" + alg + ".out")
			except IOError:
				times[alg].append(None)
				times_std[alg].append(None)
				continue
...
for e in e_list:
	...
	# Read file
	fp1 = open(inFileName + "_e" + str(e) + "_Count.out")
	line = fp1.readline()
	while line:
...
for i in range(len(algorithms)):
	...
	x_axis = [str(e / 1000.0) for e in e_list]
	ax.plot(x_axis, times[alg], label=alg_label, marker = markers[i], markersize = markersizes[i])
...
ax2.plot(x_axis, counts, linestyle="dashdot", color=suf_color)
...
for i in range(len(algorithms)):
	...
	if idx != 0 and found_None and alg != "psql" and alg != "sysx":
		if False:
			...
			ax.text((idx + idx-1) / 1.9, times[alg][idx - 1] - 0.3, "OOM", color = cmap(i), size=17)
...
if k_to_plot == 1000:
	ax.set(xlabel="$\epsilon$", ylabel="TT($10^3$) sec")
	ax.xaxis.label.set_size(22)
else:
	ax.set(xlabel="$\epsilon$", ylabel="TT(" + str(k_to_plot) + ") sec")
#plt.title(title)
ax.grid()
