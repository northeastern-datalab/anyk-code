...
public class MainEntryPoint {
    ...
    public static void main(String args[]) throws IOException {
        ...
        // ==== The following parameters can alternatively be specified in the param_file ====
        ...
        Option timings_file_path_opt = new Option("t", "timings_output_file", true, "path to timings file to be written");
        ...
        Option timing_frequency_opt = new Option("tf", "timing_frequency", true, "Record time every X answers returned");
        timing_frequency_opt.setRequired(false);
        options.addOption(timing_frequency_opt);

        Option timing_measurements_opt = new Option("tm", "timing_measurements", true, "How many time measurements to make (requires -ers)");
        ...
        Option path_optimization_opt = new Option("po", "path_optimization", false, "turn on if query is a path for better performance");
        ...
        if (jsonOption != null) result_file_path = jsonOption.getResult_Output_File();
        if (cmd.hasOption("result_output_file")) result_file_path = cmd.getOptionValue("result_output_file");

        String timings_file_path = null;
        if (jsonOption != null) timings_file_path = jsonOption.getTimings_Output_File();
        if (cmd.hasOption("timings_output_file")) timings_file_path = cmd.getOptionValue("timings_output_file");

        Integer max_k = Integer.MAX_VALUE;
        if (jsonOption != null && jsonOption.getMax_k() != null) max_k = jsonOption.getMax_k();
        if (cmd.hasOption("max_k")) max_k = Integer.parseInt(cmd.getOptionValue("max_k"));

        Double weight_cutoff = Double.MAX_VALUE;
        if (jsonOption != null && jsonOption.getWeight_cutoff() != null) weight_cutoff = jsonOption.getWeight_cutoff();
        if (cmd.hasOption("weight_cutoff")) weight_cutoff = Double.parseDouble(cmd.getOptionValue("weight_cutoff"));
        ...
        if (!alg_set_by_user) 
            System.out.println("Algorithm not specified in parameters file or command line. Defaulting to Anyk-Part-Quick.");


        Integer timing_frequency = 1;
        Integer timing_measurements = -1;
        if (jsonOption != null && jsonOption.getTiming_measurements() != null) timing_measurements = jsonOption.getTiming_measurements();
        if (cmd.hasOption("timing_measurements")) timing_measurements = Integer.parseInt(cmd.getOptionValue("timing_measurements"));
        Integer estimated_result_size = -1;
        if (jsonOption != null && jsonOption.getEstimated_result_size() != null) estimated_result_size = jsonOption.getEstimated_result_size();
        if (cmd.hasOption("estimated_result_size")) estimated_result_size = Integer.parseInt(cmd.getOptionValue("estimated_result_size"));
        ...
        if (jsonOption != null && jsonOption.getTiming_frequency() != null) timing_frequency = jsonOption.getTiming_frequency();
        if (cmd.hasOption("timing_frequency")) timing_frequency = Integer.parseInt(cmd.getOptionValue("timing_frequency"));

        String factorization_method = null;
        if (jsonOption != null) factorization_method = jsonOption.getFactorization_method();
        if (cmd.hasOption("factorization_method")) factorization_method = cmd.getOptionValue("factorization_method");

        boolean path_optimization = false;
        if (jsonOption != null && jsonOption.getPath_optimization() != null)
        {
            if (jsonOption.getPath_optimization().equals("yes")) path_optimization = true;
            else if (jsonOption.getPath_optimization().equals("no")) path_optimization = false;
            else {
                System.err.println("Invalid value for path_optimization in parameters file.");
                System.exit(1);
            } 
        }
        if (cmd.hasOption("path_optimization")) path_optimization = true;

        Configuration conf = new Configuration();
            
        // ======= Run the query =======
        // Keep results in memory, then write to a file at the end to not include I/O in elapsed time
        ...
        {
            System.err.println("Path optimization not supported yet!");
            System.exit(1);
        }
        ...
        if (algorithm.equals("Boolean")) {
            ...
            if (path_optimization) {
                ...
                for (int k = 1; k <= max_k; k++) {
                    ...
                    {
                        String result = t.toString() + "|" + t.cost;
            ...
            } else yann = new YannakakisSorting(tree_query);
        ...
        if (result_file_path != null)
        {
            ...
            for (String record: resultList) openCsv.writeLine(record);
            openCsv.flushWriter();
            openCsv.closeWriter();
        }
    }
